...
def draw_plot(
    ...
    if pytorch_results != None:
        ...
        plt.plot(xpytorch, ypytorch, label=f"Pytorch: 1-16 rdzeni")

    if onnx_results != None:
        xonnx, yonnx = cal_results(onnx_results)
        plt.plot(xonnx, yonnx, label=f"Pytorch: 1 rdzeń")

    # if tensorflow_results != None:
    #     xtensorflow, ytensorflow = cal_results(tensorflow_results)
    #     plt.plot(xtensorflow, ytensorflow, label=f"Pytorch - 600")
